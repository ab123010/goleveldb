...
由一个或多个数据块，一个可选的过滤器块，一个metaindex块，索引块和表页脚组成
Metaindex block用来保留表格参数，例如过滤器块名称及其块句柄
Index block用来保留data blocks长度和偏移量记录。
index block使用的key时最后的preceding block的key，adjacent blocks的较短分割符或最后一个块的最后一个键的较短后继。
Filter block是可选块，包含由filter generator生成的过滤器数据序列
...
Table data structure:
	...
	每个block后跟一个5字节的尾部，其中包含压缩类型和校验和。
...
Table footer:
	...
	magic为SHA-1的第一个64-bit

NOTE: All fixed-length integer are little-endian.
	所有固定长度的整数都是little-endian。
*/
...
block由一或多键值对和一个区块尾部构成
block entry与之前的键共享键前缀，直到到达重启点
一个block至少由一个重启点
第一个重启点总为0
